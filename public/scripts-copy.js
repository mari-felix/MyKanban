...
function closeModal() {
    ...
    } else if ($table.style.display === "flex" && $container.style.display === "none") {
...
async function generateCards() {
    ...
    tasks.forEach((task) => {
        ...
        formattedDue = formattedDue[2] + '/' + formattedDue[1] + '/' + formattedDue[0];
        const $columnBody = document.querySelector(`[data-column="${task.status}"] .body`);

        console.log($columnBody);

        const card = `
            <div class="card" id="${task.id}" onclick="openModalEditTask(${task.id})" draggable="true" ondragstart="dragstartHandler(event)">
                <div class="info">
...
async function createTask() {
    ...
    const newTask = {
        ...
        AssigneeId: Number($assigneeInput.value),
        Title: $titleInput.value,
        Status: Number($columnInput.value),
...
async function updateAssignee() {
    ...
    const res = await fetch(`${urlVisualStudio}Assignee/EditAssignee/${assignee.Id}`,{
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
